...
func pascalProto(s string) string {
	words := strings.Split(s, "_")
	for i, w := range words {
		words[i] = rules.Capitalize(w)
	}
...
func title(s string) string {
	logrus.Error(strings.ToTitle(strings.ToLower(s)))
...
func fileProtoName(name string) string {
	return strings.ToLower(rules.Singularize(pascal(name))) + ".proto"
}

func fileGoMapName(name string) string {
	return strings.ToLower(rules.Singularize(pascal(name))) + ".map.go"
}
