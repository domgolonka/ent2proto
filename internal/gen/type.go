...
func NewType(c *Config, schema *load.Schema) (*Type, error) {
	...
	if (!typ.ID.UserDefined) {
		ID := &Field{
			Name:      "id",
			Type:      c.IDType,
			StructTag: structTag("id", ""),
		}
		typ.Fields = append(typ.Fields, ID)
	}
	return typ, nil
}
...
// IsTime returns true if the tables has field with timestamp field.
func (t Type) HasTime() bool {
	for _, f := range t.Fields {
		if f.Type.Type == field.TypeTime {
			return true
		}
	}
	return false
...
// IsTime returns true if the table has field with a timestamp field.
func (t Type) HasUUID() bool {
	for _, f := range t.Fields {
		if f.IsUUID() {
			return true
		}
...
func (t Type) GenEdges() []*Edge {
	m := make([]*Edge, 0)
	for _, e := range t.Edges {
		//logrus.Errorf("Edge name, Table name %s %s", e.Type.Name, t.Name)
		if e.OwnFK() && e.Type.Name == t.Name || e.IsInverse(){
			continue
		}

		m = appendIfMissingEdge(m, e)
	}
	return m
}

func appendIfMissingEdge(slice []*Edge, e *Edge) []*Edge {
	for _, ele := range slice {
		if ele.Name == e.Name {
			logrus.Errorf("element name, edge name: %s %s", ele.Name, e.Name)
			return slice
		}
	}
...
// Import returns the edges names in proto form for template import
func (t Type) ImportEdges() []string {
	m := make([]string, 0)

	for _, f := range t.Edges {
		if f.Type.Name == t.Name || f.IsInverse(){
			continue
		}
		m = appendIfMissing(m, fileProtoName(f.Owner.Name))
	}
	return m
}
...
func (f Field) StructFieldID() string {
	return strings.Title(strings.ToLower(f.Name))
}
...
// StructField returns the struct member of the field in the model.
func (f Field) StructFieldProto() string {
	return pascalProto(f.Name)
}
...
func (f Field) DoNillable() string {
	if (f.IsNillable()) {
		return "*"
	}
	return ""
...
// IsInt returns true if the field is an int field.
func (f Field) IsFloat() bool { return f.Type != nil && f.Type.Type == field.TypeFloat64 }

func (f Field) IsNillable() bool { return f.Type != nil && f.Nillable }
...
//  Returns the Title name of the field.
func (f Field) Title() string {
	return strings.Title(pascal(f.Name))
}

//  Returns the Label name of the field.
...
//  Returns the Pascal name of the field.
func (f Field) Pascal() string {
	return pascal(f.Name)
}
...
func (f Field) ProtobufType() (expr string) {
	switch f.Type.Type {
	case field.TypeUUID:
		return "UUID"
	case field.TypeJSON, field.TypeBytes:
...
func (e Edge) HasUnique() bool {
	return e.OwnFK() && e.Unique
}
...
func (e Edge) StructPluralField() string {
	return pascal(rules.Pluralize(e.Name))
}
