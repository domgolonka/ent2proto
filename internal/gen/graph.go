...
func (g *Graph) Gen() (err error) {
	...
	var (
		written             []string
		templates, _ = g.templates()
	)

	b := bytes.NewBuffer(nil)
	check(templates.ExecuteTemplate(b, "uuid", g.Nodes), "load imports")
	target := filepath.Join(g.Config.Target, "uuid.proto")
	...
	for _, n := range g.Nodes {
		...
		for _, tmpl := range Templates {
			b := bytes.NewBuffer(nil)
			//todo error with the Templates
			check(templates.ExecuteTemplate(b, tmpl.Name, n), "execute template %q", tmpl.Name)
			target := filepath.Join(g.Config.Target, fileProtoName(n.Name))
			check(ioutil.WriteFile(target, b.Bytes(), 0644), "write file %s", target)
			written = append(written, target)
		}
	}
	for _, n := range g.Nodes {
		...
		//path := filepath.Join(g.Config.Target, n.Package())
		//check(os.MkdirAll(path, os.ModePerm), "create dir %q", path)
		for _, tmpl := range MapperTemplate {
			b := bytes.NewBuffer(nil)
			//todo error with the Templates
			check(templates.ExecuteTemplate(b, tmpl.Name, n), "execute template %q", tmpl.Name)
			target := filepath.Join(g.Config.Target, "map", fileGoMapName(n.Name))
			check(ioutil.WriteFile(target, b.Bytes(), 0644), "write file %s", target)
			written = append(written, target)
		}
	}
...
// had to chnange this for Protobuf!
