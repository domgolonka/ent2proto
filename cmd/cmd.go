...
import (
	...
	"github.com/domgolonka/ent2proto/internal/generate"
		...
		func() *cobra.Command {
			var (
				...
				cmd      = &cobra.Command{
					...
					Args: cobra.ExactArgs(1),
					Run: func(cmd *cobra.Command, path []string) {
						//opts := []generate.Option{entc.Storage(storage)}
						opts := []generate.Option{}
						for _, tmpl := range template {
							opts = append(opts, generate.TemplateDir(tmpl))
						}
						...
						err := generate.Generate(path[0], &cfg, opts...)
						failOnErr(err)
					},
				}
			)
